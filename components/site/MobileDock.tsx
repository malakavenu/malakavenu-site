...
export function MobileDock() {
  ...
  const itemCls = (id: string, extra = '') =>
    `dock-item${extra ? ' ' + extra : ''}${isActive(id) ? ' active' : ''}`;
  const ariaCurrent = (id: string) => (isActive(id) ? 'true' : undefined);
  ...
  return (
    <nav className="mobile-dock" id="mobileDock" aria-label="Mobile quick navigation">
      <div className="mobile-dock-inner">
        ...
        <Link
          href="/#skills"
          className={itemCls('skills')}
          aria-label="Skills"
          aria-current={ariaCurrent('skills')}
          data-section="skills"
        >
          <svg
            viewBox="0 0 24 24"
        ...
        <Link
          href="/articles"
          className={itemCls('writing')}
          aria-label="Writing"
          aria-current={ariaCurrent('writing')}
